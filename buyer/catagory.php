...
$cat=$_GET['x'];
$countsql="select count(*) as no from book_ad,books where books.book_id=book_ad.book_id and books.category='$cat'";
...
if($_SERVER["REQUEST_METHOD"]=="GET")
{
  ...
  {
    $cat=$_GET['x'];
    $sql1="SELECT book_ad.ad_id,books.book_name,books.cover_photo,books.author,book_ad.quantity,book_ad.price,user.user_name,book_ad.location FROM book_ad,books,user WHERE books.book_id=book_ad.book_id and user.user_name=book_ad.user_name and books.category='$cat' limit $i,$perPage";
  }
...
<div class="" align="center">
  <?php if($i<=0){echo "<!--";} ?>
  <a href="catagory.php?i=<?php echo $i-$perPage; ?>">Prev</a> <?php if($i<=0){echo "-->";} ?> <?php if($i+$perPage>=$total){echo "<!--";} ?><a href="catagory.php?i=<?php echo $i+$perPage; ?>">next</a><?php if($i+$perPage>=$total){echo "-->";} ?>
</div>
